...
4) generatedImagePath：string，AI 生成图片的相对路径（存于 imagedb/）
...
示例
{
  ...
  "generatedImagePath": "backend/imagedb/2025/11/18/ab/cd/abcdef123456.webp",
...
- 建议定期备份 `backend/data/` 与 `backend/imagedb/`；生产可切换到对象存储/云盘。

评分体系与汇总规则
一、单题评分
- 每个维度的基础分范围为 [-2, 2]；前端以 5 星展示，星级与分值的映射：分值 = 星级 - 3（因此 0 分对应 3 星）。
- 默认展示题目中指定的维度，默认分值为 0（即 3 星）。
- 评估过程中允许“临时增加维度”，该维度仅在本次评估中生效，不会写回题库；历史回看会显示这些临时维度及其分数。

二、维度汇总分（DimensionScores）
- 计算口径：维度分 = 该维度在本次评估中所有题目的“分值求和” ÷ 本次试题集的题目总数。
- 本次试题集的题目总数，优先以所选试题集的 `questionIds.length` 为准；若不可得，则退化为本次评估已记录条目的数量。
- 维度集合的确定：优先使用本次试题集的 `dimensionIds` 作为“参与汇总的维度集合”；若不可得，则退化为“实际出现过评分的维度集合”。
- 临时增加的维度：仅在“参与汇总的维度集合”中才会参与维度分与总分的统计；否则只体现在题目级记录与历史回看中。

三、总分（TotalScore）
- 总分 = 维度分的算术平均，即：总分 = sum(各维度分) ÷ 参与汇总的维度数量。
- 若没有任何评分数据，总分记为 0。

四、取整与显示
- 后端在计算维度分与总分时，保留 3 位小数（四舍五入）。
- 前端显示时间采用本地时区的本地化格式；分数直接展示后端返回值。
