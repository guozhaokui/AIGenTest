...
```bash
curl http://localhost:6020/api/images/62a8e8b2f2c090f754c0d1099fee3574
```
...
```bash
curl -X POST http://localhost:6020/api/images/62a8e8b2.../descriptions \
...
```bash
curl "http://localhost:6020/api/images?offset=0&limit=20&status=ready"
```
...
```bash
# 软删除
curl -X DELETE http://localhost:6020/api/images/62a8e8b2...

# 硬删除（同时删除文件）
curl -X DELETE "http://localhost:6020/api/images/62a8e8b2...?hard=true"
```
...
1. 初始化 Conda 环境 (hidream)
2. 启动文本嵌入服务 (端口 6011)
3. 启动图片嵌入服务 (端口 6010)
4. 启动图片管理服务 (端口 6020)
5. 启动 Node.js 后端 (端口 3000)
6. 启动前端开发服务器 (端口 5173)
7. 等待服务启动并进行健康检查
