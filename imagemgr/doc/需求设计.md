...
SigLIP2 是一个跨模态模型，其文本嵌入和图片嵌入在同一向量空间。因此：

- **普通文本模型**（如 BGE、Qwen3）：文本嵌入 → 在**文本索引**中搜索
- **SigLIP2**：文本嵌入 → 在**图片索引**中搜索（跨模态）

```
用户输入: "一只猫在晒太阳"，选择 siglip2_text_v1 索引
         ↓
1. 使用 SigLIP2 计算文本嵌入（1152维）
         ↓
2. 在 siglip2_image_v1 索引中搜索（不是文本索引！）
         ↓
3. 直接匹配图片视觉特征，无需图片有描述
```

**代码位置**：`api_server.py` 的 `search_by_text` 函数中有特殊判断：
...
> ⚠️ **注意**：如果后续添加新的跨模态图片嵌入模型（如 CLIP、EVA 等），需要在此处添加类似的特殊处理，使其文本嵌入能够在对应的图片索引中搜索。
...
defaults:
  ...
  text_embedding: bge_local  # BGE 专为检索设计
```
...
|------|------|------|
| 图片管理服务 | 6060 | 主服务 |
| 图片嵌入服务 | 6010 | SigLIP2（1152维） |
| 文本嵌入服务 | 6011 | Qwen3-4B（2560维，备选） |
| 文本嵌入服务 | 6012 | **BGE-large-zh（1024维，默认）** |

### 文本嵌入模型选择

| 模型 | 维度 | 特点 | 推荐场景 |
|------|------|------|----------|
| **BGE-large-zh** | 1024 | 专为检索设计，区分度好 | ✅ 默认使用 |
| Qwen3-4B | 2560 | 指令增强后效果接近 BGE | 备选 |

**Qwen3 优化**：使用提示词 `"关于"` 增强检索效果，区分度从 0.00 提升到 0.147。

### 两阶段检索（可选）

对于需要更高精度的场景，可以使用 LLM 重排序：

```
第一阶段：向量快速召回 top-100
         ↓
第二阶段：LLM 判断相关性，重排为 top-10
```

**重排序服务**（端口 6013，可选启动）：

```bash
# 启动时加上 --with-rerank 参数
./start_embed_server.sh --with-rerank
```

API 调用示例：
```bash
curl -X POST http://GPU_SERVER:6013/rerank \
  -H "Content-Type: application/json" \
  -d '{
    "query": "猫",
    "documents": ["一只小猫在睡觉", "狗在公园奔跑", "汽车飞驰"],
    "top_k": 2
  }'
```
...
```json
{
  "query": "猫",
  "results": [
    {"document": "一只小猫在睡觉", "score": 0.92, "original_index": 0},
    {"document": "狗在公园奔跑", "score": 0.45, "original_index": 1}
  ]
}
```
